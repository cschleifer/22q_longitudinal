...
# install longCombat with devtools
...
```{r}
# function to print path for each cifti being read
read_cifti_print <- function(path){
  print(path)
  out <- read_cifti(path,brainstructures="all")
  return(out)
}
```

## load FC results
load trio GBC results
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
#
#gbc_name <- "resting_fc_gbc_s_hpss_res-mVWMWB1d_lpss_gbc_mFz_0.dtseries.nii"
#
## read gbc results
#trio_gbc <- lapply(trio_sessions, function(s)  read_cifti(file.path(trio_dir,s,"images/functional/",gbc_name),brainstructures="all"))
```

get vector of average gbc for each session
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
#trio_gbc_mean <- lapply(trio_gbc, function(g) mean(as.matrix(g))) %>% do.call(rbind,.) %>% as.data.frame
#df_gbc_mean <- cbind(as.data.frame(trio_sessions),trio_gbc_mean)
#colnames(df_gbc_mean) <- c("session","wb_gbc_mean")
#
```

load trio thal FC results
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# paths to sessions directories
trio_dir <- file.path(hoffman,"22q/qunex_studyfolder/sessions")
...
```{r}
thal_name <- "resting_fc_seed_s_hpss_res-mVWMWB1d_lpss_LR-Thal-CIFTI_all_Fz.dtseries.nii"

trio_thal <- lapply(trio_sessions, function(s)  read_cifti_print(file.path(trio_dir,s,"images/functional/",thal_name)))

```


load trio hipp FC results
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# paths to sessions directories
trio_dir <- file.path(hoffman,"22q/qunex_studyfolder/sessions")
...
hipp_name <- "resting_fc_seed_s_hpss_res-mVWMWB1d_lpss_LR-Hipp-CIFTI_all_Fz.dtseries.nii"

trio_hipp <- lapply(trio_sessions, function(s)  read_cifti_print(file.path(trio_dir,s,"images/functional/",hipp_name)))

```


load prisma thal FC results
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# paths to sessions directories
...
```{r}
prisma_thal_name <- "resting1_fc_seed_s_hpss_res-mVWMWB1d_lpss_LR-Thal-CIFTI_all_Fz.dtseries.nii"

prisma_thal <- lapply(prisma_sessions, function(s)  read_cifti_print(file.path(prisma_dir,s,"images/functional/",prisma_thal_name)))

```

load prisma hipp FC results
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# paths to sessions directories
prisma_dir <- file.path(hoffman,"22qPrisma/qunex_studyfolder/sessions")
...
prisma_hipp_name <- "resting1_fc_seed_s_hpss_res-mVWMWB1d_lpss_LR-Hipp-CIFTI_all_Fz.dtseries.nii"

prisma_hipp <- lapply(prisma_sessions, function(s)  read_cifti_print(file.path(prisma_dir,s,"images/functional/",prisma_hipp_name)))

```

combine trio and prisma lists
```{r}
all_thal <- c(trio_thal,prisma_thal)
all_hipp <- c(trio_hipp,prisma_hipp)
```
...
## Ji (Anticevic Cole) network parcellation
...
function to extract average value in roi
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
extract_mask_mean_wb <- function(xii_target, xii_mask, mask_vals){
  inds <- which(as.matrix(xii_mask) %in% mask_vals)
  ext <- as.matrix(xii_target)[inds]
  out <- mean(ext)
  return(out)
}

extract_mask_mean_cortex <- function(xii_target, xii_mask, mask_vals){
  indsl <- which(as.matrix(xii_mask$data$cortex_left) %in% mask_vals)
  indsr <- which(as.matrix(xii_mask$data$cortex_right) %in% mask_vals)
  extl <- as.matrix(xii_target$data$cortex_left)[indsl]
  extr <- as.matrix(xii_target$data$cortex_right)[indsr]
  ext <- c(extl,extr)
  out <- mean(ext)
  return(out)
}
...
extract thal to network FC for all networks
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# cortex only
#thal_nets_cortex <- lapply(1:nrow(ji_net_keys), function(r) lapply(trio_thal, function(g) extract_mask_mean_cortex(xii_target=g, xii_mask=xii_Ji_network, mask_vals=ji_net_keys$NETWORKKEY[r])) %>% do.call(rbind,.) %>% as.data.frame) %>% do.call(cbind,.) %>% as.data.frame

#names(thal_nets_cortex) <- paste("thal_",(lapply(ji_net_keys$NETWORK,function(x) sub("-","_",x)) %>% do.call(rbind,.)), "_cortex", sep="")

# whole-brain
#thal_nets_wb <- lapply(1:nrow(ji_net_keys), function(r) lapply(trio_thal, function(g) extract_mask_mean_wb(xii_target=g, xii_mask=xii_Ji_network, mask_vals=ji_net_keys$NETWORKKEY[r])) %>% do.call(rbind,.) %>% as.data.frame) %>% do.call(cbind,.) %>% as.data.frame
thal_nets_wb <- lapply(1:nrow(ji_net_keys), function(r) lapply(all_thal, function(g) extract_mask_mean_wb(xii_target=g, xii_mask=xii_Ji_network, mask_vals=ji_net_keys$NETWORKKEY[r])) %>% do.call(rbind,.) %>% as.data.frame) %>% do.call(cbind,.) %>% as.data.frame

names(thal_nets_wb) <- paste("thal_",(lapply(ji_net_keys$NETWORK,function(x) sub("-","_",x)) %>% do.call(rbind,.)), "_wb", sep="")

#df_thal_fc <- cbind(thal_nets_cortex, thal_nets_wb)
df_thal_fc <-  thal_nets_wb

#df_thal_fc$session <- trio_sessions
df_thal_fc$session <- all_sessions

```


extract hipp to network FC for all networks
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# whole-brain
#hipp_nets_wb <- lapply(1:nrow(ji_net_keys), function(r) lapply(trio_hipp, function(g) extract_mask_mean_wb(xii_target=g, xii_mask=xii_Ji_network, mask_vals=ji_net_keys$NETWORKKEY[r])) %>% do.call(rbind,.) %>% as.data.frame) %>% do.call(cbind,.) %>% as.data.frame
hipp_nets_wb <- lapply(1:nrow(ji_net_keys), function(r) lapply(all_hipp, function(g) extract_mask_mean_wb(xii_target=g, xii_mask=xii_Ji_network, mask_vals=ji_net_keys$NETWORKKEY[r])) %>% do.call(rbind,.) %>% as.data.frame) %>% do.call(cbind,.) %>% as.data.frame

names(hipp_nets_wb) <- paste("hipp_",(lapply(ji_net_keys$NETWORK,function(x) sub("-","_",x)) %>% do.call(rbind,.)), "_wb", sep="")

#df_thal_fc <- cbind(thal_nets_cortex, thal_nets_wb)
df_thal_hipp_fc <-  cbind(df_thal_fc,hipp_nets_wb)
